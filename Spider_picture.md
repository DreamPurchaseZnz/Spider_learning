...
def get_urls():
    """ 获取网站下所有图的 url
    """
    page_urls = ['http://www.207vod.com/Html/63/index-{cnt}.html'.format(cnt=cnt)
    ...
    for page_url in page_urls:
        try:
            ...
            img_urlr = [url.replace("/Html", 'https://www.303sp.com/Html') for url in img_urlc]
    ...
    try:
        ...
        with lock:
            if make_dir(folder_name):
                ...
                img_urls = [url.replace("/Uploads", 'https://www.207vod.com/Uploads') for url in img_urls]
...
def delete_empty_dir(dir):
    ...
    但此时文件夹已经存在所以会忽略该图的下载，此时要删除空文件夹
    """
